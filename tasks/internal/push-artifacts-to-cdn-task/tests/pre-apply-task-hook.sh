...
kubectl create secret generic pulp-task-exodus-secret --from-literal=cert=SENSITIVE_DATA_myexoduscert --from-literal=key=SENSITIVE_DATA_myexoduskey --from-literal=url=SENSITIVE_DATA_https://exodus.com

# Create a dummy pulp secret (and delete it first if it exists)
kubectl delete secret pulp-task-pulp-secret --ignore-not-found
kubectl create secret generic pulp-task-pulp-secret --from-literal=konflux-release-rhsm-pulp.crt=SENSITIVE_DATA_mypulpcert --from-literal=konflux-release-rhsm-pulp.key=SENSITIVE_DATA_mypulpkey --from-literal=pulp_url=SENSITIVE_DATA_https://pulp.com
...
kubectl create secret generic pulp-task-bad-pulp-secret --from-literal=konflux-release-rhsm-pulp.crt=SENSITIVE_DATA_mypulpcert --from-literal=konflux-release-rhsm-pulp.key=SENSITIVE_DATA_mypulpkey --from-literal=pulp_url=SENSITIVE_DATA_https://failing-pulp.com

# Create a dummy udc secret (and delete it first if it exists)
kubectl delete secret pulp-task-udc-secret --ignore-not-found
kubectl create secret generic pulp-task-udc-secret --from-literal=cert=SENSITIVE_DATA_myudccert --from-literal=key=SENSITIVE_DATA_myudckey --from-literal=url=SENSITIVE_DATA_https://udc.com
...
kubectl create secret generic "checksum-credentials" \
    --from-literal=keytab="SENSITIVE_DATA_" \
    --from-literal=user="SENSITIVE_DATA_konflux-release-signing-sa" \
    --from-literal=host="SENSITIVE_DATA_etera-worker.hosted.upshift.rdu2.redhat.com" \
    --from-literal=fingerprint="SENSITIVE_DATA_$(ssh-keygen -lf "${TMPDIR}/checksum.pub")"

# create quay, windows and mac secrets
kubectl create secret generic quay-credentials --from-literal=username="SENSITIVE_DATA_testuser" --from-literal=password="SENSITIVE_DATA_testpass"
kubectl create secret generic windows-credentials --from-literal=username="SENSITIVE_DATA_testuser" --from-literal=port="SENSITIVE_DATA_1234" --from-literal=host="SENSITIVE_DATA_testhost"
kubectl create secret generic mac-host-credentials --from-literal=username="SENSITIVE_DATA_testuser" --from-literal=host="SENSITIVE_DATA_testhost"
