...
# contain at least one SENSITIVE_DATA_ string when using --from-literal

set -euo pipefail

fail=0

echo "Checking that kubectl create secret commands contain SENSITIVE_DATA_ in pre-apply-task-hook.sh files"
...
for file in $files; do
  ...
  while IFS=: read -r line_num command; do
    ...
    if echo "$clean_command" | grep "kubectl create secret" | grep -q -- "--from-literal"; then
      ...
      # Check if command contains SENSITIVE_DATA_ directly
      if echo "$clean_command" | grep -q "SENSITIVE_DATA_"; then
      ...
      if echo "$clean_command" | grep -q '\$'; then
        ...
        # Check if any of these variables are defined with SENSITIVE_DATA_ in the file
        found_sensitive=0
        for var in $var_names; do
          # Look for variable assignment: var_name=... or var_name=$(...)
          # Extract the variable definition and check if it contains SENSITIVE_DATA_
          ...
          if [[ -n "$var_def" ]] && echo "$var_def" | grep -q "SENSITIVE_DATA_"; then
            found_sensitive=1
            break
          fi
        done
        ...
        if [[ $found_sensitive -eq 1 ]]; then
          continue  # Variable contains SENSITIVE_DATA_, command is OK
        fi
      fi

      # If we reach here, the command doesn't have SENSITIVE_DATA_
      echo "ERROR: $file line $line_num: kubectl create secret with --from-literal does not contain SENSITIVE_DATA_"
    ...
    if echo "$clean_command" | grep -q -- "--from-file"; then
      ...
      for filename in $filenames; do
        ...
        if [[ -n "$file_creation" ]]; then
          # Check if the file creation command contains SENSITIVE_DATA_
          if ! echo "$file_creation" | grep -q "SENSITIVE_DATA_"; then
            echo "ERROR: $file line $line_num: kubectl create secret with --from-file uses a file without SENSITIVE_DATA_"
            echo "  Command: $(echo "$command" | tr '\n' ' ' | sed 's/  */ /g')"
...
if [[ $fail -ne 0 ]]; then
  echo ""
  echo "FAILED: Some kubectl create secret commands are missing SENSITIVE_DATA_ prefix"
  exit 1
fi

echo ""
echo "SUCCESS: All kubectl create secret commands with --from-literal contain SENSITIVE_DATA_"
